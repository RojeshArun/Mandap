...
public class SupplierOrdersActivity extends BaseActivity implements
		OnItemClickListener {
	...
	private void initiateSuppliersListWS() {
		...
		params.putString(
				...
				String.valueOf(UserDetails.getInstance(
						SupplierOrdersActivity.this).getUserId()));
		JsonArrayRequest mJsonRequestResponse = new JsonArrayRequest(
				StaticUtils.encodeUrl(WSClass.SUPPLIER_MY_ORDERS, params),
	...
	private void loadAllData(JSONArray response) {
		...
		for (int i = 0; i < response.length(); i++) {
			...
			try {
				...
				if (mObj.has(StaticUsage.SENDER_NAME)) {
					mDataHolder.setProductName(mObj
							.getString(StaticUsage.SENDER_NAME));
				}
